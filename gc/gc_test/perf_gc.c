...
// static void gcmap_print(GarbageCollector* gc_)
// {
//     int total=0;
//     for (size_t i=0; i<gc_->allocs->capacity; ++i) {
//         Allocation* chunk = gc_->allocs->allocs[i];
//         while (chunk) {
//             LOG_INFO("Checking ptr: %p", chunk->ptr);
...
//         }
//     }
//     LOG_INFO("total ptr: %d", total);
//     LOG_INFO("total capacity: %d",gc_->allocs->capacity);
//     // LOG_INFO("from space: %p - %p",gc_->heap.from_space, gc_->heap.from_space_end);
//     // LOG_INFO("to space: %p - %p",gc_->heap.from_space, gc_->heap.from_space_end);
//     LOG_INFO("from space: %p",gc_->heap.from_space);
//     LOG_INFO("to space: %p",gc_->heap.to_space);
//     return;
// }
...
static char* test_gc_basic_alloc_free()
{
     ...
     * collected.
     */
    printf("begin test_gc_basic_alloc_free\n");
    DTOR_COUNT = 0;
    GarbageCollector gc_;
    void *bos = __builtin_frame_address(0);
