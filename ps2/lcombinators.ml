...
struct
  ...
  (* [mlcomment] matches ML-style comments that begin with (* and end with *).  
   * However, note that unlike ML, this won't handle nested comments!  Can
   * you transform it so it does? *)
  let mlcomment : (char,unit) parser = 
      unit(seq ((seq(c '(', c '*')), 
             seq(star (alts [unit(notC '*'); unit(seq(c '*',notC ')'))]),
                 (seq(c '*', c ')')))))

  (* [ccomment] matches C-style comments that begin with /* and end with */. *)
  let ccomment : (char,unit) parser = 
      unit(seq ((seq(c '/', c '*')), 
             seq(star (alts [unit(notC '*'); unit(seq(c '*',notC '/'))]),
                 (seq(c '*', c '/')))))

  (* [whitespace] matches zero or more C-style comments, spaces, tabs, etc. *)
  let whitespace = unit(star (alt (ccomment,white)))
